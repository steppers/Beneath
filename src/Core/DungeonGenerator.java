    ...
    public static TileMap CreateDungeon(int width, int height, DungeonType type, int difficulty){
        ...
        while(true){
            ...
            if(tiles[x][y].type == Tile.TileType.STONE_FLOOR){
                ...
                if(p.length() > 20){
                    if(!tiles[x-1][y].isSolid && !tiles[x+1][y].isSolid && !tiles[x][y-1].isSolid && !tiles[x][y+1].isSolid && !tiles[x-1][y-1].isSolid && !tiles[x-1][y+1].isSolid && !tiles[x+1][y-1].isSolid && !tiles[x+1][y+1].isSolid){
