...
public class GUI {
    ...
    private static Rectangle pause = new Rectangle(10, 542, 48, 48);
    ...
    public static void update(Player player, StateBasedGame sbg, GameContainer gc, TurnManager tm) throws SlickException {
        ...
        switch(state){
            case IN_GAME:
                ...
                if(GUI.info.contains(input.getMouseX(), input.getMouseY()) && input.isMousePressed(0) || input.isKeyPressed(Input.KEY_I)){
                    state = GUIState.INFO;
                ...
                if(GUI.inv.contains(input.getMouseX(), input.getMouseY()) && input.isMousePressed(0) || input.isKeyPressed(Input.KEY_E)){
                    state = GUIState.INVENTORY;
                    AudioBank.playEffect(AudioBank.Select2);
                }
                if(GUI.pause.contains(input.getMouseX(), input.getMouseY()) && input.isMousePressed(0)){
            ...
            case INVENTORY:
                ...
                if(input.isMousePressed(0)){
                    ...
                    if(GUI.inv12.contains(input.getMouseX(), input.getMouseY())){
                        selected = ItemManager.getInvItem(11);
                        if(selected != null)
                            AudioBank.playEffect(AudioBank.Select2);
                        xSel = 3; ySel = 3;
                    }
                    ...
                    if(GUI.inv.contains(input.getMouseX(), input.getMouseY()) || input.isKeyPressed(Input.KEY_E)){
                        state = GUIState.IN_GAME;
                        selected = null;
                        AudioBank.playEffect(AudioBank.Select2);
                    }
    ...
    private static void renderPause(Graphics g, Player p) {
        g.setColor(GUIBacking);
        //box
        g.setLineWidth(2);
        g.fillRoundRect(300, 285, 200, 35, 6);
        g.setColor(Color.gray);
        g.drawRoundRect(300, 285, 200, 35, 6);

        g.setColor(Color.white);
        g.drawString("---PAUSED---", 350, 295);
    }
    ...
    private static void renderInventory(Graphics g) {
        ...
        if(ItemManager.playerWeapon != null && ItemManager.playerWeapon.enchantment != null){
            ItemManager.playerWeapon.image.draw(193, 40, 96, 96, new Color(1,0.7f+ Enchantment.getFilter(),1));
        }else if(ItemManager.playerWeapon != null){
            ItemManager.playerWeapon.image.draw(193, 40, 96, 96);
        }
        if(ItemManager.playerArmour != null && ItemManager.playerArmour.enchantment != null){
            ItemManager.playerArmour.image.draw(299, 40, 96, 96, new Color(1,0.7f+Enchantment.getFilter(),1));
        }else if(ItemManager.playerArmour != null){
            ItemManager.playerArmour.image.draw(299, 40, 96, 96);
        }
        if(ItemManager.playerJewel1 != null && ItemManager.playerJewel1.enchantment != null){
            ItemManager.playerJewel1.image.draw(405, 40, 96, 96, new Color(1,0.7f+Enchantment.getFilter(),1));
        }else if(ItemManager.playerJewel1 != null){
            ItemManager.playerJewel1.image.draw(405, 40, 96, 96);
        }
        if(ItemManager.playerJewel2 != null && ItemManager.playerJewel2.enchantment != null){
            ItemManager.playerJewel2.image.draw(511, 40, 96, 96, new Color(1,0.7f+Enchantment.getFilter(),1));
        }else if(ItemManager.playerJewel2 != null){
            ItemManager.playerJewel2.image.draw(511, 40, 96, 96);
        }

        int x = 0, y = 1;
        for(Item i : ItemManager.inventoryItems){
            if(i.enchantment != null)
                i.image.draw(193 + (x*96) + (x*10), 40 + (y*96) + (y*10), 96, 96, new Color(1,0.7f+Enchantment.getFilter(),1));
            else
    ...
    private static void drawSelectedData(Graphics g) {
        g.setColor(GUIBacking);
        g.fillRoundRect(-12, 175, 170, 250, 12);
        g.setColor(Color.gray);
        g.drawRoundRect(-12, 175, 170, 250, 12);
        ...
        if(selected.enchantment != null)
            g.drawString(selected.enchantment.name, 5, 200);
        g.drawString("ATK:  " + selected.stats.Attack, 5, 220);
        g.drawString("DEF:  " + selected.stats.Defense, 5, 240);
        g.drawString("+HP:  " + selected.stats.HP, 5, 260);
        if(selected.stats.Speed == 1){
            g.drawString("SPD:  FAST", 5, 280);
        }
        else if(selected.stats.Speed == 2){
            g.drawString("SPD:  SLOW", 5, 280);
        }else{
            g.drawString("SPD:  N/A", 5, 280);
        }
        g.drawString("EVA:  " + selected.stats.Evade, 5, 300);

        if(selected.enchantment != null){
            g.setColor(Color.green);
            if(selected.enchantment.Attack >0)
                g.drawString("+(" + selected.enchantment.Attack + ")", 100, 220);
            if(selected.enchantment.Defense >0)
                g.drawString("+(" + selected.enchantment.Defense + ")", 100, 240);
            if(selected.enchantment.maxHP >0)
                g.drawString("+(" + selected.enchantment.maxHP + ")", 100, 260);
            if(selected.enchantment.Evade >0)
                g.drawString("+(" + selected.enchantment.Evade + ")", 100, 300);
        }
    ...
    private static void renderInGame(Graphics g, Player p, int level){
        ...
        if(ItemManager.playerWeapon != null){
            if(ItemManager.playerWeapon.enchantment != null)
                ItemManager.playerWeapon.image.draw(740, 542, 48, 48,  new Color(1,0.7f+Enchantment.getFilter(),1));
            else
        ...
        float width = 270*((float)p.stats.HP/(float)p.stats.maxHP);
        g.fillRect(520, 9, width, 10);

        if((width/270)*100 < 30){
            g.drawString("LOW HEALTH!!", 140, 560);
        }

        g.setColor(Color.yellow);
        g.drawString("XP", 497, 25);
        width = 270*(float)(p.exp/p.targetExp);
    ...
    private static void renderInfo(Graphics g, Player p){
        ...
        //info box
        g.setLineWidth(2);
        g.fillRoundRect(300, 200, 200, 200, 12);
        g.setColor(Color.gray);
        g.drawRoundRect(300, 200, 200, 200, 12);

        g.setColor(Color.white);
        g.drawImage(ImageBank.player.getSubImage(4, 1, 8, 7), 310, 210, 366, 258, 0, 0, 8, 7);
        g.drawString(p.playerClass, 385, 229);
        g.drawString("HP : " + p.stats.HP + "/" + p.stats.maxHP, 310, 270);
        g.drawString("EXP: " + (int)p.exp + "/" + (int)p.targetExp, 310, 290);
        g.drawString("ATK: " + p.stats.Attack, 310, 330);
        g.drawString("DEF: " + p.stats.Defense, 310, 350);
        g.drawString("EVA: " + p.stats.Evade, 310, 370);

        g.drawString("KILLS: " + p.kills, 390, 330);
    }
